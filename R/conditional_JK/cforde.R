...
cforde <- function(params_uncond,cond) {
  ...
  list(condition = condition, volumes = cond, prob_condition = sum(cvg_new_unnormalized)/max(params_uncond$forest$tree), cnt = cnt_new, cat = cat_new, forest = forest_new, meta = meta, input_class_x = params_uncond$input_class, input_class_c = class(cond))
}
