...
As expected, all values in the maximum likelihood calculation are shrunk toward a uniform prior when we impose Laplace smoothing---unless a category is the only one permitted in a leaf by the preceding splits, in which case it occurs with probability 1. Note also that the two data tables do not have exactly the same rows. This is because we omit zero probability events in the former case to conserve memory. However, we can verify that probabilities sum to unity for each leaf-variable combination.
