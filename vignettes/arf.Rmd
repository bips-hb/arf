...
A note on **nested parallelism**: when a backend is registered, ARF distributes work across worker processes, and within each worker `data.table` may *also* use several threads. On a machine with many cores these multiply (`workers × threads`), and if the product exceeds your cores the result is oversubscription --- which *slows things down* rather than speeding them up, and inflates load average because `data.table`'s idle OpenMP threads busy-wait by default. If parallel performance disappoints or load spikes, cap per-worker threads with `data.table::setDTthreads()` and set the environment variable `OMP_WAIT_POLICY=passive` (before starting R) so idle threads sleep instead of spinning. The best split of cores between worker processes and per-worker threads depends on your data and hardware.
