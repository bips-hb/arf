...
If both are available, set `options(arf.backend = "foreach")` or `"mirai"` to choose explicitly. See `?"arf-options"` for details. The same backend is used by `forde`, `forge`, `expct`, and `lik`. For reproducible parallel sampling, start seeded daemons with `daemons(2, seed = 42)` and keep the daemon count fixed; see `?"arf-options"`.
