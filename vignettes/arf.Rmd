...
As an alternative to `foreach`, arf can use the `mirai` backend. Start daemons and arf uses them automatically, with no `foreach` registration needed. The learned circuit is shared across workers via `mori`, so memory stays roughly flat in the number of workers instead of growing with it. Speed is otherwise comparable. This matters most for many workers or large circuits.
